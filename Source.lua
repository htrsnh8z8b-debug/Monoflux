-- ===================================================================
--  БИБЛИОТЕКА LINORIA (модифицированная: cloneref + getService)
-- ===================================================================

-- ====== УНИВЕРСАЛЬНОЕ ПОЛУЧЕНИЕ СЕРВИСОВ (с поддержкой cloneref) ======
local function getService(serviceName)
    -- Пытаемся найти cloneref в глобальной области или через syn
    local clonerefFunc = getgenv().cloneref or (syn and syn.cloneref)
    if clonerefFunc then
        return clonerefFunc(game:GetService(serviceName))
    else
        return game:GetService(serviceName)
    end
end

-- ====== ЗАМЕНА ВСЕХ СТАРЫХ ВЫЗОВОВ ======
...
function Library:MakeDraggable(Instance, Cutoff, IsWindow)
    ...
    Instance.InputBegan:Connect(function(Input)
        if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
            if IsWindow and InputService.TouchEnabled and not _G.UIUnlocked then return end
...
-- ====== НОВЫЕ УВЕДОМЛЕНИЯ: BOTTOM CENTER ======
